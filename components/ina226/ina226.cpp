...
// N·zvy alert funkciÌ - musia presne zodpovedaù hodnot·m v sensor.py
...
void INA226Component::setup() {
  ...
  delay(1);  // »akaj na dokonËenie resetu

  // 2) Nastav konfigur·ciu
  config.raw                          = 0;
  config.reserved                     = 0b100;  // podæa datasheetu
  ...
  // 3) VypoËÌtaj a zapÌö kalibr·ciu
  // LSB pr˙du = max_current / 2^15, uloûenÈ v mikroampÈroch
  ...
  // 4) Inicializuj alert registre do bezpeËnÈho stavu
...
void INA226Component::update() {
  // V shutdown mÛde neËÌtame merania
  if (this->shutdown_)
    return;
  ...
  if (this->bus_voltage_sensor_ != nullptr) {
    ...
    // Bus voltage je vûdy kladn˝, LSB = 1.25 mV
  ...
  if (this->shunt_voltage_sensor_ != nullptr) {
    ...
    // Shunt voltage mÙûe byù z·porn˝ (dvojkov˝ doplnok), LSB = 2.5 µV
  ...
  if (this->current_sensor_ != nullptr) {
    ...
    // Pr˙d v ampÈroch: raw * LSB [µA] / 1 000 000
  ...
  if (this->power_sensor_ != nullptr) {
    ...
    // V˝kon v wattoch: raw * 25 * LSB [µA] / 1 000 000
    // Power LSB = 25 ◊ Current_LSB (podæa datasheetu sekcia 6.5.1)
...
// ===============================================================================
// Shutdown - z·pis do Configuration registra
// MODE bits = 000 alebo 100 õ Power-Down
// MODE bits = 111            õ Shunt and Bus, Continuous
// ===============================================================================
void INA226Component::set_shutdown(bool shutdown) {
  this->shutdown_ = shutdown;
  this->update_config_register_();

  // Informuj switch o novom stave (dÙleûitÈ pre spr·vne zobrazenie v HA)
  if (this->shutdown_switch_ != nullptr) {
    this->shutdown_switch_->publish_state(shutdown);
  }

  ESP_LOGI(TAG, "INA226 shutdown: %s", shutdown ? "ON (power-down)" : "OFF (running)");
}

// ===============================================================================
// Alert function select - z·pis do Mask/Enable registra (06h)
// ===============================================================================
...
// ===============================================================================
// Alert limit number - z·pis do Alert Limit registra (07h)
// ===============================================================================
...
// PrepÌöe Configuration register podæa aktu·lneho stavu shutdown_
// ===============================================================================
void INA226Component::update_config_register_() {
  ...
  if (!this->write_byte_16(INA226_REGISTER_CONFIG, config.raw)) {
    ESP_LOGE(TAG, "Chyba z·pisu do Configuration registra");
    this->status_set_warning();
  }
}
...
// ZapÌöe Mask/Enable (06h) a Alert Limit (07h) podæa aktu·lneho nastavenia
// ===============================================================================
void INA226Component::update_alert_registers_() {
  ...
  if (!this->write_byte_16(INA226_REGISTER_MASK_ENABLE, mask)) {
    ESP_LOGE(TAG, "Chyba z·pisu do Mask/Enable registra");
    this->status_set_warning();
    return;
  }

  // 2) Alert Limit register - nastav prahov˙ hodnotu
  uint16_t raw_limit = this->alert_limit_to_raw_(this->alert_limit_);

  if (!this->write_byte_16(INA226_REGISTER_ALERT_LIMIT, raw_limit)) {
    ESP_LOGE(TAG, "Chyba z·pisu do Alert Limit registra");
    this->status_set_warning();
    return;
  }
...
// Prevedie n·zov funkcie na hodnotu pre Mask/Enable register (06h)
// ===============================================================================
uint16_t INA226Component::alert_function_to_mask_(const std::string &function) {
  ...
  return 0x0000;  // None - ûiadny alert
}

// ===============================================================================
// alert_limit_to_raw_()
// Prevedie fyzik·lnu hodnotu na raw hodnotu pre Alert Limit register (07h)
//
// Pravidl· konverzie (datasheet sekcia 7.1.8):
//   SOL/SUL: Shunt Voltage õ LSB = 2.5 µV õ raw = limit_V / 0.0000025
//   BOL/BUL: Bus Voltage   õ LSB = 1.25 mV õ raw = limit_V / 0.00125
//   POL:     Power         õ Power_LSB = 25 ◊ Current_LSB [µA] / 1e6
...
// Volan· z Home Assistant keÔ pouûÌvateæ prepne switch
// ===============================================================================
void INA226ShutdownSwitch::write_state(bool state) {
  if (this->parent_ == nullptr)
    return;
  this->parent_->set_shutdown(state);
  // publish_state je volanÈ v set_shutdown() õ nie tu, aby sme prediöli
  // dvojitÈmu volaniu
}

// ===============================================================================
// INA226AlertSelect::control()
// Volan· z Home Assistant keÔ pouûÌvateæ vyberie alert funkciu
// ===============================================================================
...
// Volan· z Home Assistant keÔ pouûÌvateæ zmenÌ alert limit
// ===============================================================================
